...
import { Plus, ClipboardList, Trash2, Loader2, Banknote } from 'lucide-react';
import { toast } from 'sonner';
import { useNavigate } from '@tanstack/react-router';
import { useLanguage } from '@/contexts/LanguageContext';
import type { Transaction, CreditPaymentTransaction } from '../backend';
...
export default function CustomerDetailPage() {
  ...
  const unifiedEntries: UnifiedEntry[] = [
    ...transactions.map((tx): UnifiedEntry => ({ type: 'transaction', date: tx.date, data: tx })),
    ...creditPayments.map((cp): UnifiedEntry => ({ type: 'creditPayment', date: cp.transactionDate, data: cp })),
  ...
  const handleDeleteCreditPayment = async (paymentId: bigint) => {
    ...
    try {
      await deleteCreditPayment.mutateAsync({ paymentId, customerId: customerIdBig });
          ...
          {showPayForm && (
            <form onSubmit={handlePayCreditDue} className="space-y-3">
              <div className="space-y-1.5">
                <Label htmlFor="pay-amount" className="text-blue-800 font-semibold text-sm">
                  {t('paymentAmount')}
                </Label>
                <Input
                  id="pay-amount"
