...
import { Calendar, IndianRupee, Phone } from "lucide-react";
...
export default function CustomerDashboard({
  ...
  const dateCreated = new Date(
    Number(customer.dateCreated) / 1_000_000,
    ...
    <Card className="bg-gradient-to-br from-lemon-yellow to-lemon-yellow-dark/60 border-lemon-yellow-dark/40 shadow-md mb-5">
      <CardContent className="p-5">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className="text-lemon-dark font-extrabold text-xl leading-tight">
              {customer.name}
            </h2>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white/60 rounded-xl p-3 text-center">
            ...
            <p
              className={`text-2xl font-extrabold ${hasCredit ? "text-orange-600" : "text-lemon-green-dark"}`}
            >
              ₹{balance.toFixed(2)}
            </p>
            <p className="text-xs text-lemon-dark/50 mt-0.5">
              {hasCredit ? t("outstanding") : t("allClear")}
            </p>
          </div>
          <div className="bg-white/60 rounded-xl p-3 text-center">
            <div className="flex items-center justify-center gap-1 mb-1">
