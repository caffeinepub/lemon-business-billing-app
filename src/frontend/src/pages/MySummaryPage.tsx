...
import { useGetAllCustomers, useGetLemonSummary } from "@/hooks/useQueries";
import {
  Citrus,
  IndianRupee,
  TrendingDown,
    ...
    <Card className={`${bgClass} ${borderClass} shadow-sm`}>
      <CardContent className="p-4">
        <div className="flex items-center gap-3">
          <div
            className={`w-10 h-10 rounded-full flex items-center justify-center ${bgClass} border ${borderClass}`}
          >
            {icon}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-xs text-lemon-dark/60 font-medium">{label}</p>
            <p className={`text-xl font-extrabold ${colorClass} leading-tight`}>
              {value}
            </p>
          </div>
        </div>
...
export default function MySummaryPage() {
  ...
  return (
    <div>
      {/* Page Header */}
      <div className="mb-5">
        <h2 className="text-xl font-extrabold text-lemon-dark">
          {t("myLemonSummaryTitle")}
        </h2>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          ...
          {/* Profit / Loss */}
          <Card
            className={`shadow-sm ${isProfit ? "bg-lemon-green/15 border-lemon-green/40" : "bg-red-50 border-red-200"}`}
          >
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center ${isProfit ? "bg-lemon-green/20 border border-lemon-green/40" : "bg-red-100 border border-red-200"}`}
                >
                  {isProfit ? (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p
                    className={`text-xl font-extrabold leading-tight ${isProfit ? "text-lemon-green-dark" : "text-red-600"}`}
                  >
                    {isProfit ? "+" : "-"}₹
                ...
                <span
                  className={`text-xs font-bold px-2 py-1 rounded-full ${isProfit ? "bg-lemon-green/20 text-lemon-green-dark" : "bg-red-100 text-red-600"}`}
                >
