...
import { BarChart3, Download, Loader2, LogIn, Plus, Users } from "lucide-react";
...
// ── CustomerRow ───────────────────────────────────────────────────────────────
// Lean row — no per-row backend calls to avoid N+1 slowness on the home page.
// Balance and last-tx date are displayed on the customer detail page instead.
...
const CustomerRow = memo(function CustomerRow({
  ...
  return (
    <CustomerListItem
      customer={customer}
      balance={customer.previousCredit}
...
function LoginScreen() {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center gap-6">
      ...
      <div>
        <h2 className="text-2xl font-extrabold text-lemon-dark mb-2">
          Welcome Back!
        </h2>
    ...
    <Dialog open>
      ...
      >
        <DialogHeader>
          ...
          <DialogDescription>
            Enter your name to get started with Lemon Business.
          </DialogDescription>
        </DialogHeader>
...
export default function HomePage() {
  ...
  const { t } = useLanguage();
  const { canInstall, showIOSInstructions, promptToInstall } = usePWAInstall();
  ...
  return (
    <div>
      ...
      {summaryLoading ? (
        ...
        <SummaryBanner customerCount={customers.length} summary={summary} />
      ) : null}
      ...
      <Link to="/my-summary" className="block mb-5">
        <div className="flex items-center gap-3 bg-lemon-yellow/60 hover:bg-lemon-yellow border border-lemon-yellow-dark/30 rounded-xl px-4 py-3 transition-colors cursor-pointer">
          ...
          <div className="flex-1">
            <p className="font-bold text-lemon-dark text-sm">
              {t("myLemonSummary")}
            </p>
            <p className="text-xs text-lemon-dark/60">
              {t("myLemonSummaryDesc")}
            </p>
          </div>
