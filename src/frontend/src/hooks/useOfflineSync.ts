...
export function useOfflineSync() {
    ...
    async (op: OfflineOperation): Promise<boolean> => {
      ...
      try {
        if (op.type === "addCustomer") {
          ...
          const { customerId, lemonQuantity, ratePerUnit, todayDebited } =
            op.payload as {
              customerId: string;
          ...
          queryClient.invalidateQueries({
            queryKey: ["creditPayments", customerId],
          });
          queryClient.invalidateQueries({ queryKey: ["lemonSummary"] });
        }
        return true;
        ...
        console.warn(
          "[OfflineSync] Failed to process operation:",
          op.type,
          err,
        );
        return false;
  ...
  const syncPending = useCallback(async () => {
    if (isSyncingRef.current || !actor || !identity || !navigator.onLine)
      return;
